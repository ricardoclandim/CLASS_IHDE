...
int perturb_init(
  ...
  double w_fld_ini, w_fld_0,dw_over_da_fld,integral_fld, lambda_cdm, lambda_de, Omega0_cdm, Omega0_fld;
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    class_call(background_w_fld(pba, 0., pba->lambda_cdm,pba->lambda_de,pba->Omega0_cdm,pba->Omega0_fld,  &w_fld_ini,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
    class_call(background_w_fld(pba,pba->a_today, pba->lambda_cdm,pba->lambda_de,pba->Omega0_cdm,pba->Omega0_fld, &w_fld_0,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
 /* END MODIFICATION RL*/
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* BEGIN MODIFICATION RL   */
        ...
        /* cdm velocity vanishes in the synchronous gauge */
             ...
             else if(pba->iDMDE_pert_type == DEDM){ppw->pv->y[ppw->pv->index_pt_delta_cdm] = 3./4.*(1.-pba->lambda_cdm/3.- pba->lambda_de/3.*pba->Omega0_fld/pba->Omega0_cdm)*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* cdm density */
        /* cdm velocity vanishes in the synchronous gauge */
        }  
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        class_call(background_w_fld(pba,a,pba->lambda_cdm,pba->lambda_de,pba->Omega0_cdm,pba->Omega0_fld,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
        ...
        if (pba->use_ppf == _FALSE_) {
          ...
          if(pba->has_iDMDE == _TRUE_){
            ...
            else if(pba->iDMDE_pert_type == DEDM){ppw->pv->y[ppw->pv->index_pt_delta_fld] = 3./4.*(1.-pba->lambda_cdm/3.- pba->lambda_de/3.*pba->Omega0_fld/pba->Omega0_cdm)*ppw->pv->y[ppw->pv->index_pt_delta_g];
            ppw->pv->y[ppw->pv->index_pt_theta_fld] = ppw->pv->y[ppw->pv->index_pt_theta_g];
          	}  
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_theta_fld == _TRUE_) {
      ...
      class_call(background_w_fld(pba,a_rel*pba->a_today, pba->lambda_cdm,pba->lambda_de,pba->Omega0_cdm,pba->Omega0_fld, &w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
  ...
  if (_scalars_) {
      ...
      if (ppt->gauge == synchronous) {
        ...
        if(pba->has_fld == _TRUE_){
          ...
          if(pba->iDMDE_pert_type == DiValentino){
            ...
            dy[pv->index_pt_delta_cdm] = -y[pv->index_pt_theta_cdm]
                                         -metric_continuity
                                         +pba->lambda_de*pba->Omega0_fld/pba->Omega0_cdm*(
                                                a_prime_over_a*(y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_cdm]))
                                                 +(pba->lambda_cdm +pba->lambda_de*pba->Omega0_fld/pba->Omega0_cdm)*
                                                 (metric_continuity/3
                                                 +((
                                               (1.+1./3.)*pvecback[pba->index_bg_rho_g]/pvecback[pba->index_bg_rho_tot]*theta_g
                                                 +(1.+w_ihde)*pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_tot]*y[pv->index_pt_theta_fld]
                                                +pvecback[pba->index_bg_rho_b]/pvecback[pba->index_bg_rho_tot]*y[pv->index_pt_theta_b]
                                                )/(1.+1./3.+w_ihde)
                                                 )/3.)
                                         ;
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        if(pba->has_iDMDE == _TRUE_){
          ...
          /* BEGIN MODIFICATION RL added -(pba->delta_Q)*(metric_continuity/3 +k vt/3). in the line above*/
            ...
            //        Eq. (2.2c) of Murgia et al. 2016 (typo in last term)
            double w_ihde = -1/3*(pba->lambda_cdm+ pba->lambda_de*pba->Omega0_fld/pba->Omega0_cdm)*(1.+pba->Omega0_cdm/pba->Omega0_fld);
            dy[pv->index_pt_delta_fld] = -(1+w_ihde)*(y[pv->index_pt_theta_fld]+metric_continuity)
                                         -3.*a_prime_over_a*(1.-w_ihde)*(
                                                  y[pv->index_pt_delta_fld]
                                                  +a_prime_over_a*y[pv->index_pt_theta_fld]/k2*(
                                                                3.*(1.+w_ihde)
                                                                +pba->lambda_de
                                                                +pba->lambda_cdm*pba->Omega0_cdm/pba->Omega0_fld
                                                                )
                                                  )
                                        +a_prime_over_a*pba->lambda_cdm*pba->Omega0_cdm/pba->Omega0_fld*(y[pv->index_pt_delta_fld]
                                        			-y[pv->index_pt_delta_cdm])          
                                       -(pba->lambda_cdm*pba->Omega0_cdm/pba->Omega0_fld + pba->lambda_de)*(metric_continuity/3.
                                       	     +((
                                               (1.+1./3.)*pvecback[pba->index_bg_rho_g]/pvecback[pba->index_bg_rho_tot]*theta_g
                                                 +(1.+w_ihde)*pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_tot]*y[pv->index_pt_theta_fld]
                                                +pvecback[pba->index_bg_rho_b]/pvecback[pba->index_bg_rho_tot]*y[pv->index_pt_theta_b]
      ...
      /* BEGIN MODIFICATION RL */     
            ...
            dy[pv->index_pt_theta_fld] = 2*a_prime_over_a*y[pv->index_pt_theta_fld]*(
                                                1.
                                                +pba->lambda_de/(1+w_ihde)
                                                +pba->lambda_cdm*(pba->Omega0_cdm/pba->Omega0_fld)/(1+w_ihde)
                                                )
                                        +k2/(1.+w_ihde)*y[pv->index_pt_delta_fld];
          }
          else if(pba->iDMDE_pert_type == IntDM){
            // Here we use the equations as they are given in
